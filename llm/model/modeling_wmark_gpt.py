...
from llm.model.modeling_llama import LlamaConfig, LlamaModel, LlamaForCausalLM
from transformers import AutoConfig, AutoModelForCausalLM, AutoTokenizer, CLIPImageProcessor
from transformers.modeling_outputs import CausalLMOutputWithPast

from llm.model.configuration_wmark_gpt import MPLUGOwl2Config, MplugOwlVisionConfig, MplugOwlVisualAbstractorConfig
from llm.model.visual_encoder import MplugOwlVisionModel, MplugOwlVisualAbstractorModel
IGNORE_INDEX = -100
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if images is None or input_ids.shape[1] == 1:
            ...
            if past_key_values is not None and images is not None and input_ids.shape[1] == 1:
                ...
                attention_mask = torch.ones((attention_mask.shape[0], input_ids.shape[1]), dtype=attention_mask.dtype, device=attention_mask.device)
...
class MPLUGOwl2LlamaForCausalLM(LlamaForCausalLM, MPLUGOwl2MetaForCausalLM):
    ...
    def __init__(self, config):
        ...
        self.preferential_ids_ = [id_[1] for id_ in self.tokenizer(['invisible', 'faint', 'moderate', 'visible', 'obvious'])["input_ids"]]
        self.post_init()

        if config.name_or_path:
            self.load_pretrain_weights(config.name_or_path)
    ...
    @torch.no_grad()
    def generate_from_logists(self,
        ...
        predict_ids = torch.argmax(probabilities, dim=-1)
        predict_ids = predict_ids.flatten()
        generate_ids = []
        for step in range(len(predict_ids)):
            generate_ids.append(predict_ids[step].item())
        generate_ids.append(eos_token_id)
        return generate_ids
...
if __name__ == "__main__":
    ...
    config = MPLUGOwl2Config.from_pretrained('/home/aiseon/storage1/huggingface/MAGAer13-mplug-owl2-llama2-7b')
    config.use_cache = False
    config.num_hidden_layers = 8
    config.name_or_path = '/home/aiseon/storage1/huggingface/MAGAer13-mplug-owl2-llama2-7b'
    model = MPLUGOwl2LlamaForCausalLM(config).to(torch.bfloat16).to(device)

    ent = torch.Tensor([0] * 40 + [1] * 100 + [0] * (203-40-100)).to(torch.int64).unsqueeze(dim=0)

    input_ids = torch.randint(1, 20, (8, 203)).to(device)
    attention_mask = torch.concat([ent for i in range(8)], dim=0)
    attention_mask = attention_mask.bool()
    labels = torch.randint(1, 20, (8, 203)).to(device)
    images = torch.randn(8, 3, 448, 448).to(torch.bfloat16).to(device)
    while True:
        y = model(input_ids=input_ids, attention_mask=attention_mask, labels=labels, images=images)
        # y = model.encode_images(images)
        print(y.shape)
        print('')